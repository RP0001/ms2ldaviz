#!/usr/bin/env python
import textwrap #just wraps text
from argparse import ArgumentParser, FileType, RawDescriptionHelpFormatter #cmd interface
import json #json encoder and decoder
import os
import sys #os and system imports
...
# for now use a single core will suffice - the below env variables are set to specifically use a single core

os.environ["MKL_NUM_THREADS"] = "1"
os.environ["NUMEXPR_NUM_THREADS"] = "1"
os.environ["OMP_NUM_THREADS"] = "1"

# the system path is set for windows, use / for linux - appended manually
...
# logging is necessary to keep track
import logging
logging.basicConfig(format='%(asctime)s : %(levelname)s : %(message)s', level=logging.DEBUG)

# sequence of commands needed to run gensim is here from cmd
def build_parser():
    ...
    # CORPUS - this just adds the relevant arguments in the consoles
    ...
    # LDA - changed to 0 default as it can therefore use a single core
    ...
    lda.add_argument('--workers', type=int, default=0, help='Number of workers. 0 will use single core LdaCore otherwise will use LdaMulticore (default: %(default)s)')
    ...
    # insert - lda results inserted into the database
    ...
    # insert - gensim lda results inserted into the database
